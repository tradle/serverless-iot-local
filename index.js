...
// Custom implementation of functionHelper.getFunctionOptions
const getFunctionOptions = (fun, key, servicePath) => {
  return {
    functionName: key,
    handlerName: fun.handler.split('.').pop(),
    handlerPath: path.join(servicePath, fun.handler.split('.').slice(0, -1).join('.')),
    runtime: fun.runtime,
    timeout: fun.timeout
  }
}
...
const createHandler = (options, serverlessOptions) => {
  const { handlerPath, handlerName } = options

  // Clear require cache if skipCacheInvalidation is not true
  if (!serverlessOptions.skipCacheInvalidation) {
    Object.keys(require.cache).forEach(key => {
      if (key.includes(handlerPath)) {
  ...
  try {
    ...
    throw new Error(`Error while loading ${handlerPath}: ${err.message}`)
  }
}
...
class ServerlessIotLocal {
  ...
  _createMQTTClient () {
    ...
    Object.keys(this.service.functions).forEach(key => {
      ...
      const funOptions = getFunctionOptions(fun, key, servicePath)
      ...
      matches.forEach(topicMatcher => {
        ...
        functions.forEach(fnInfo => {
          ...
          try {
            ...
            handler = createHandler(options, this.options)
          } catch (err) {
